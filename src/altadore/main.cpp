...
int main() {
  ...
  scoped_refptr<ShapeNode> sphere1(new ShapeNode(new Sphere(), new Material(new Color(1.0, 0.0, 1.0), 25, 0.25)));
  root->AddChild(sphere1.ptr());

  scoped_refptr<ShapeNode> sphere2(new ShapeNode(new Cube(), new Material(new Color(0.0, 1.0, 0.0), 15, 0)));
  scoped_refptr<TransformNode> transform(new TransformNode());
  transform->Scale(0.2);
